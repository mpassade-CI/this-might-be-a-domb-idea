const par = document.querySelector('p')
par.style.color = 'lightblue'

const head = document.querySelector('h1')
head.style.fontSize = '5em'

par.innerText = 'Lorem ipsum dolor amet viral meh selfies drinking vinegar, intelligentsia poke flannel twee paleo enamel pin cray. Banjo celiac crucifix, kickstarter la croix air plant jianbing hashtag vinyl hell of man bun selvage schlitz banh mi. Tacos hella raclette quinoa blog, williamsburg adaptogen tbh. Hexagon af stumptown lumbersexual synth gentrify quinoa enamel pin celiac master cleanse. Truffaut typewriter shoreditch, semiotics iceland mixtape taxidermy umami distillery austin hashtag. Food truck synth wayfarers, street art banh mi actually authentic. Bitters tousled tattooed vegan neutra pug hell of fixie chia unicorn letterpress.'

const item13 = document.querySelector('#item-13')
item13.style.opacity = '0.5'

const item3 = document.querySelector('#item-3')
item3.innerText = 'I say, "Hi!"'

const image = document.querySelector('img')
image.src = 'http://www.tioxic.com/wp-content/uploads/trex_4.jpg'
image.height = '300'

const x = document.createElement('img')
const y = document.body.appendChild(x)
y.src = 'https://previews.123rf.com/images/gorbick/gorbick1804/gorbick180400001/100123869-flying-pterodactyl-dino-dangerous-prehistoric-creature.jpg'
y.height = '300'

const newItem = document.createElement('li')
const list = document.querySelector('ul')
const item16 = list.appendChild(newItem)
item16.classList.add('item')
item16.id = 'item-16'
item16.innerText = "Won't get fooled again."